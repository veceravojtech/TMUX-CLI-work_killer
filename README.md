...
One command — `./scripts/release.sh` (patch bump by default; `--minor`, `--major`,
`--dry-run`, `--watch`). See [Releasing in docs/advanced-usage.md](docs/advanced-usage.md#releasing)
for the full workflow, the CI release pipeline, and deploy-credential rotation.
