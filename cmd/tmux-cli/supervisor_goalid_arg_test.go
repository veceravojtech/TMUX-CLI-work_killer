...
func TestSupervisorXml_Step0bConsumesGoalIDArgument(t *testing.T) {
	...
	// Scope to just the argument branch and assert SUPERVISOR_WID is read from
	// the daemon's per-goal supervisor-window marker (byte-exact, written before
	// boot), NOT synthesized as supervisor-{ns} from the goal id and NOT probed
	// via the unreliable tmux display-message active-window call. Post-P1 a goal
	// supervisor is ALWAYS supervisor-{ns}; the bare `supervisor` denotes ONLY
	// window-0 / a standalone interactive supervisor (never a goal window), so
	// reading the marker verbatim routes worker [EXECUTE:DONE] replies to the real
	// spawned window. The goal id governs scoping (research-root / tasks-path);
	// the marker content governs routing.
	...
	assert.Contains(t, argBranch, "supervisor-window",
		"the arg branch must read SUPERVISOR_WID from the per-goal supervisor-window marker, not probe it")
	...
	assert.Contains(t, argBranch, "window-0",
		"the arg branch must call out that bare `supervisor` is window-0/standalone only, so a goal SUPERVISOR_WID is always the namespaced supervisor-{ns}")
}
