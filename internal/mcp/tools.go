...
// Implements FR7: AI assistants can monitor multiple window outputs sequentially to detect errors
//
// This method validates the window identifier, resolves it to a window ID, then uses the
// tmux.Executor to capture pane content directly (Decision 3: Direct internal calls).
// It returns raw plain text output as a string (Decision 4: Go types directly).
//
// Window Identifier Resolution:
// - If identifier starts with "@" (e.g., "@0", "@1"), it's treated as a window ID
// - Otherwise, it's treated as a window name and resolved via case-sensitive exact match
...
// Examples:
//
//	WindowsCapture("@0")        // Capture output from window ID @0
//	WindowsCapture("supervisor") // Capture output from window named "supervisor"
//
// Parameters:
//   - windowIdentifier: Either a window ID (starts with "@") or window name (must be non-empty)
//
// Returns:
//   - string: Captured pane content as plain text (empty string if pane has no output)
//   - error: ErrInvalidWindowID if windowIdentifier is empty
//     ErrSessionNotFound if session file cannot be loaded
//     ErrWindowNotFound if window ID or name doesn't exist in session
//     ErrTmuxCommandFailed if tmux capture-pane command fails
//
// Performance: Expected to complete in <2s for normal output (NFR-P1),
//
//	<5s for large output >10,000 lines (NFR-P5).
func (s *Server) WindowsCapture(windowIdentifier string) (string, error) {
	// Validate window identifier format (STRICT Rule 9)
	if windowIdentifier == "" {
		return "", fmt.Errorf("%w: windowIdentifier cannot be empty", ErrInvalidWindowID)
	}
	...
	if err != nil {
		// Use categorized error with context (STRICT Rule 9)
		return "", fmt.Errorf("%w in directory %s: expected %s",
			ErrSessionNotFound, s.workingDir, s.sessionFile)
	}
	...
	// Verify window exists in session before attempting capture
	...
	if !windowExists {
		// Window not found - return categorized error with context (STRICT Rule 9)
		return "", fmt.Errorf("%w: window=%s session=%s",
			ErrWindowNotFound, windowID, session.SessionID)
	}

	// Capture pane output using tmux.Executor (direct internal call per Decision 3)
	output, err := s.executor.CaptureWindowOutput(session.SessionID, windowID)
	if err != nil {
		// Wrap tmux execution error with categorized error and context (STRICT Rule 9)
		return "", fmt.Errorf("%w: session=%s window=%s: %w",
			ErrTmuxCommandFailed, session.SessionID, windowID, err)
	}

	// Return raw string output directly - MCP SDK handles JSON serialization (Decision 4)
	// Empty output is valid (return empty string, not error)
	return output, nil
}
