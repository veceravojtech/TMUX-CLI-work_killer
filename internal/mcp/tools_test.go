...
// TestServer_WindowsCapture_Success_WithContent verifies that WindowsCapture
// returns pane output as plain text when window exists and has content.
func TestServer_WindowsCapture_Success_WithContent(t *testing.T) {
	// Arrange: Mock store with session, mock executor with output
	mockStore := &mockSessionStore{
		session: &store.Session{
			SessionID: "test-session",
			Windows: []store.Window{
	...
	expectedOutput := "Hello from tmux window\nLine 2\nLine 3"
	mockExecutor := &testutil.MockTmuxExecutor{}
	mockExecutor.On("CaptureWindowOutput", "test-session", "@0").Return(expectedOutput, nil)
...
// TestServer_WindowsCapture_Success_EmptyOutput verifies that WindowsCapture
// returns empty string (not error) when window has no output.
func TestServer_WindowsCapture_Success_EmptyOutput(t *testing.T) {
	// Arrange: Window with no content
	mockStore := &mockSessionStore{
		session: &store.Session{
			SessionID: "test-session",
			Windows:   []store.Window{{TmuxWindowID: "@0"}},
		},
	}

	mockExecutor := &testutil.MockTmuxExecutor{}
	mockExecutor.On("CaptureWindowOutput", "test-session", "@0").Return("", nil)
	...
	// Assert: Empty output is valid, not an error
	require.NoError(t, err)
	assert.Equal(t, "", output)
	mockExecutor.AssertExpectations(t)
}

// TestServer_WindowsCapture_Error_InvalidWindowID verifies that WindowsCapture
// returns ErrInvalidWindowID when windowID is empty.
func TestServer_WindowsCapture_Error_InvalidWindowID(t *testing.T) {
	// Arrange
	mockStore := &mockSessionStore{
		session: &store.Session{SessionID: "test-session"},
	}
...
// TestServer_WindowsCapture_Error_WindowNotFound verifies that WindowsCapture
// returns ErrWindowNotFound when window doesn't exist in session.
func TestServer_WindowsCapture_Error_WindowNotFound(t *testing.T) {
	// Arrange: Session with window @0, but request @99
	mockStore := &mockSessionStore{
		session: &store.Session{
			SessionID: "test-session",
			Windows:   []store.Window{{TmuxWindowID: "@0"}},
	...
	assert.Contains(t, err.Error(), "window=@99")
	assert.Contains(t, err.Error(), "session=test-session")
}

// TestServer_WindowsCapture_Error_SessionNotFound verifies that WindowsCapture
// returns ErrSessionNotFound when session file cannot be loaded.
func TestServer_WindowsCapture_Error_SessionNotFound(t *testing.T) {
	// Arrange: Mock store returns error
	mockStore := &mockSessionStore{
		loadError: store.ErrSessionNotFound,
	}
...
// TestServer_WindowsCapture_Error_TmuxCommandFailed verifies that WindowsCapture
// returns ErrTmuxCommandFailed when tmux capture-pane command fails.
func TestServer_WindowsCapture_Error_TmuxCommandFailed(t *testing.T) {
	...
	mockExecutor := &testutil.MockTmuxExecutor{}
	mockExecutor.On("CaptureWindowOutput", "test-session", "@0").Return("", errors.New("tmux not running"))
...
// TestServer_WindowsCapture_LargeOutput verifies that WindowsCapture
// handles large output (>1000 lines) correctly.
func TestServer_WindowsCapture_LargeOutput(t *testing.T) {
	// Arrange: Simulate large output >1000 lines
	var largeOutputBuilder string
	for i := 0; i < 1500; i++ {
		largeOutputBuilder += fmt.Sprintf("Line %d: Some content here\n", i)
	}
	largeOutput := largeOutputBuilder
	...
	mockExecutor := &testutil.MockTmuxExecutor{}
	mockExecutor.On("CaptureWindowOutput", "test-session", "@0").Return(largeOutput, nil)
	...
	// Assert
	require.NoError(t, err)
	assert.Equal(t, largeOutput, output)
	assert.Contains(t, output, "Line 1499") // Verify all lines present
	mockExecutor.AssertExpectations(t)
}
...
func TestServer_WindowsCapture_WithWindowName(t *testing.T) {
	mockStore := &mockSessionStore{
		session: &store.Session{
			SessionID: "test-session",
			Windows: []store.Window{
	...
	mockExecutor := &testutil.MockTmuxExecutor{}
	mockExecutor.On("CaptureWindowOutput", "test-session", "@0").Return("test output", nil)
	...
	output, err := server.WindowsCapture("supervisor")
	require.NoError(t, err)
