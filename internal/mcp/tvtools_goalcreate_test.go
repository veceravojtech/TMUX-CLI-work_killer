...
func TestGoalCreate_FirstGoal(t *testing.T) {
	...
	output, err := server.GoalCreate("Fix prices", []string{"Price matches API"}, []string{"Check price"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
`)
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Third", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_ExplicitMaxRetries(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Custom retries", nil, []string{"check"}, "", "", "", 5, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_DefaultMaxRetries(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Default retries", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreateDefaultMaxRetriesIsFive(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Default budgets", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_EmptyDescription(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("", nil, nil, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_DescriptionTooLong(t *testing.T) {
	...
	_, err := server.GoalCreate(longDesc, nil, nil, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_DescriptionExactlyAtLimit(t *testing.T) {
	...
	output, err := server.GoalCreate(exactDesc, nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
`)
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Second", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_AtomicWrite(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Test atomic", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_WithContext(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Refactor auth", []string{"Tests pass"}, []string{"check"}, "Legacy code", "Performance", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_WithPhase(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Setup DB", nil, []string{"check"}, "", "", "infrastructure", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_NoAcceptance(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Simple task", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_EmptyValidate(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Valid desc", nil, nil, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_EmptyValidateSlice(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Valid desc", nil, []string{}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_InvalidPhase(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Bad phase goal", nil, []string{"check"}, "", "", "nonexistent", 0, nil, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_WithDependsOn(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("First goal", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)

	output, err := server.GoalCreate("Second goal", nil, []string{"check"}, "", "", "", 0, []string{"goal-001"}, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_DependsOnNonExistent(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Orphan goal", nil, []string{"check"}, "", "", "", 0, []string{"goal-999"}, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_WithPhaseAndDependsOn(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Prereq goal", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)

	output, err := server.GoalCreate("Domain goal", nil, []string{"check"}, "", "", "domain", 0, []string{"goal-001"}, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
	assert.Equal(t, "goal-002", output.ID)
...
func TestGoalCreate_DependsOnEmptySlice(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("No deps goal", nil, []string{"check"}, "", "", "", 0, []string{}, nil, nil, nil, 0, "", false)
...
func TestGoalCreate_DependsOnMultiple(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = server.GoalCreate("Goal B", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)

	output, err := server.GoalCreate("Goal C", nil, []string{"check"}, "", "", "", 0, []string{"goal-001", "goal-002"}, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
	assert.Equal(t, "goal-003", output.ID)
...
func TestGoalCreate_LockFileCreated(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Lock test", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_LockCoversLoadSaveSpan(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_, err1 = server.GoalCreate("First concurrent", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	}()
	go func() {
		defer wg.Done()
		_, err2 = server.GoalCreate("Second concurrent", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	}()
...
`)
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	_, err := server.GoalCreate("Second goal", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_WritesPhaseToGoalMD(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Domain goal", nil, []string{"check"}, "", "", "domain", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_NoPhaseOmitsSection(t *testing.T) {
	...
	server := newTestServer(new(testutil.MockTmuxExecutor), tmpDir)
	output, err := server.GoalCreate("Simple goal", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_WithPreconditions_PersistsToGoalsYaml(t *testing.T) {
	...
	output, err := server.GoalCreate("Setup DB", nil, []string{"check"}, "", "", "infrastructure", 0, nil, preconds, nil, nil, 0, "", false)
	require.NoError(t, err)
	assert.Equal(t, "goal-001", output.ID)
...
func TestGoalCreate_PersistsScope(t *testing.T) {
	...
	output, err := server.GoalCreate("Scoped goal", nil, []string{"check"}, "", "", "", 0, nil, nil, nil, scope, 0, "", false)
	require.NoError(t, err)
	assert.Equal(t, "goal-001", output.ID)
...
func TestGoalCreate_AcceptsValidInvestigators(t *testing.T) {
	...
	output, err := server.GoalCreate("Valid investigators", nil, []string{"check"}, "", "", "", 0, nil, nil, validInvestigatorSet(2), nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_AcceptsFourInvestigators(t *testing.T) {
	...
	output, err := server.GoalCreate("Four investigators", nil, []string{"check"}, "", "", "", 0, nil, nil, validInvestigatorSet(4), nil, 0, "", false)
	require.NoError(t, err)
	assert.Equal(t, "goal-001", output.ID)
}
...
func TestGoalCreate_RejectsTooFewInvestigators(t *testing.T) {
	...
	_, err := server.GoalCreate("Too few", nil, []string{"check"}, "", "", "", 0, nil, nil, validInvestigatorSet(1), nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
...
func TestGoalCreate_RejectsTooManyInvestigators(t *testing.T) {
	...
	_, err := server.GoalCreate("Too many", nil, []string{"check"}, "", "", "", 0, nil, nil, validInvestigatorSet(5), nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
	require.ErrorContains(t, err, "2–4")
}
...
func TestGoalCreate_RejectsEmptyName(t *testing.T) {
	...
	invs[1].Name = ""
	_, err := server.GoalCreate("Empty name", nil, []string{"check"}, "", "", "", 0, nil, nil, invs, nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
...
func TestGoalCreate_RejectsBadType(t *testing.T) {
	...
	invs := validInvestigatorSet(2)
	invs[0].Type = "bogus"
	_, err := server.GoalCreate("Bad type", nil, []string{"check"}, "", "", "", 0, nil, nil, invs, nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
...
func TestGoalCreate_AcceptsPlannerEmittedTypes(t *testing.T) {
	...
	for _, plannerType := range newTypes {
		t.Run(plannerType, func(t *testing.T) {
			...
			output, err := server.GoalCreate("Planner type "+plannerType, nil, []string{"check"}, "", "", "", 0, nil, nil, invs, nil, 0, "", false)
			require.NoError(t, err)
...
func TestGoalCreate_RejectsMissingCommand(t *testing.T) {
	...
	invs := validInvestigatorSet(2)
	invs[0].Commands = nil
	_, err := server.GoalCreate("Missing command", nil, []string{"check"}, "", "", "", 0, nil, nil, invs, nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
...
func TestGoalCreate_RejectsEmptyPass(t *testing.T) {
	...
	invs := validInvestigatorSet(2)
	invs[0].Pass = ""
	_, err := server.GoalCreate("Empty pass", nil, []string{"check"}, "", "", "", 0, nil, nil, invs, nil, 0, "", false)
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrInvalidInput)
...
func TestGoalCreate_FallbackWhenNoInvestigators(t *testing.T) {
	...
	// valid 2–4 section.
	output, err := server.GoalCreate("Fallback", nil, []string{"PHPStan level 9 passes", "Unit tests pass"}, "", "", "", 0, nil, nil, nil, nil, 0, "", false)
	require.NoError(t, err)
...
func TestGoalCreate_UnchangedGuardsStillFire(t *testing.T) {
	...
	_, err := server.GoalCreate("", nil, []string{"check"}, "", "", "", 0, nil, nil, validInvestigatorSet(2), nil, 0, "", false)
	assert.ErrorIs(t, err, ErrInvalidInput)

	// Empty validate still rejects.
	_, err = server.GoalCreate("Valid desc", nil, nil, "", "", "", 0, nil, nil, validInvestigatorSet(2), nil, 0, "", false)
	assert.ErrorIs(t, err, ErrInvalidInput)
}
