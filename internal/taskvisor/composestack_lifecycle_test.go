...
// --- TC-4 / TC-5: execRuntimeForGoal --------------------------------------

func TestExecRuntimeForGoal_WorktreeOverridesProject(t *testing.T) {
	d, _, dir := setupDaemon(t)
	writeTestEnvMD(t, dir, dockerTestEnv)
	primeWorktree(d, "goal-015") // goalUsesWorktree ⇒ true

	er := d.execRuntimeForGoal(&Goal{ID: "goal-015"})

	assert.Equal(t, "taskvisor-goal-015", er.ComposeProject, "worktree goal targets its own stack")
	assert.Equal(t, "docker", er.RunTarget, "RunTarget comes from base resolution")
	assert.Equal(t, "app", er.AppSvc, "AppSvc comes from base resolution")
}

func TestExecRuntimeForGoal_NoWorktreeBaseProject(t *testing.T) {
	d, _, dir := setupDaemon(t)
	writeTestEnvMD(t, dir, dockerTestEnv)

	er := d.execRuntimeForGoal(&Goal{ID: "goal-1"}) // no worktree primed
	base := ResolveExecRuntime(dir)

	assert.Equal(t, base, er, "no-worktree runtime is byte-identical to the base resolution")
	assert.Equal(t, base.ComposeProject, er.ComposeProject, "ComposeProject stays at base")
}

// --- TC-6: regenerated validate.sh renders the worktree project -----------

func TestRegenerateValidateScript_RendersWorktreeProject(t *testing.T) {
	d, _, dir := setupDaemon(t)
	writeTestEnvMD(t, dir, dockerTestEnv)
	primeWorktree(d, "goal-015")
	goalDir, err := EnsureGoalDir(dir, "goal-015")
	require.NoError(t, err)

	goal := &Goal{ID: "goal-015", Validate: []string{"bin/console doctrine:schema:validate"}}
	require.NoError(t, d.regenerateValidateScript(goal))

	data, err := os.ReadFile(filepath.Join(goalDir, "validate.sh"))
	require.NoError(t, err)
	assert.Contains(t, string(data), "docker compose -p taskvisor-goal-015 exec -T app",
		"validate.sh execs into the per-worktree stack, not the main stack")
}

// --- TC-7 / TC-8: db-lock gate around runValidateScript -------------------

func TestRunValidateScript_WorktreeSkipsDBLock(t *testing.T) {
	d, _, dir := setupDaemon(t)
	wt := primeWorktreeDir(t, d, "goal-015") // worktree present + on disk
	goalDir, err := EnsureGoalDir(dir, "goal-015")
	require.NoError(t, err)
	...
	var gotCwd string
	d.SetScriptRunnerFunc(func(_ context.Context, _ string, cwd string, _ []string) (string, string, int, error) {
		heldDuringExec = tryDBLockHeld(t, dir)
		gotCwd = cwd
		return "", "", 0, nil
	})

	passed, _, _, err := d.runValidateScript(&Goal{ID: "goal-015"})
	require.NoError(t, err)
	assert.True(t, passed)
	assert.False(t, heldDuringExec, "a worktree goal validates WITHOUT the shared db lock (per-worktree DB isolated)")
	assert.Equal(t, wt, gotCwd, "validate runs with cwd=worktree")
}

func TestRunValidateScript_NoWorktreeHoldsDBLock(t *testing.T) {
	d, _, dir := setupDaemon(t)
	...
	// No worktree primed (WorktreeDir empty) ⇒ byte-identical locked path.
	passed, _, _, err := d.runValidateScript(&Goal{ID: "goal-001"})
	require.NoError(t, err)
	assert.True(t, passed)
	assert.True(t, heldDuringExec, "a no-worktree goal validate holds the db lock (byte-identical)")
	assert.False(t, tryDBLockHeld(t, dir), "db lock released after runValidateScript returns")
}

// --- TC-9 / TC-10 / TC-11: discardWorktree teardown -----------------------
